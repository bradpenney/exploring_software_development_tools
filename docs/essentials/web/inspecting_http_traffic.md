...
This article isn't about learning `curl` — that part's assumed. It's about learning to **see**: turning `curl` and your browser's DevTools into x-ray glasses for HTTP, so the invisible parts of a request become visible and debuggable.
...
`curl` is perfect for the request *you* construct. But some problems only exist in a real browser: most famously [CORS](https://networking.bradpenney.io/efficiency/http/cors_explained/) and the automatic requests browsers make on their own. For those, open **DevTools → Network** (F12 in the Chrome or Firefox browser).
...
??? question "Practice Problem 2: TLS or Auth?"
    ...
    ??? tip "Solution"
        ...
        Watch *where* `curl -v` stops. A **TLS/certificate** problem fails during the handshake: `*` lines about SSL and an error appear *before* any `>` request lines or `<` status line (no HTTP happened). An **authorization** problem completes the handshake and the request fully, then returns `< HTTP/1.1 403 Forbidden`: the connection was fine, the server just refused the action. TLS fails before HTTP; `403` is HTTP working and saying "no."
...
Speaking `curl` is the easy part; this is about listening as well as talking. The headers, the handshake, the redirects, the preflights: they're all right there the moment `-v` gets added or the Network tab opens. Reading the whole conversation instead of just the body is what turns an API running on faith into a system that can actually be seen.
